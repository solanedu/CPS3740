...
//sql for money table
$sqlTransact = "SELECT mid,code,type,amount,name AS Source,mydatetime,note FROM CPS3740_2020F.Money_solanedu,CPS3740.Sources WHERE id=sid AND cid IN(select id FROM CPS3740.Customers WHERE login='$user_login' )";
...
elseif ($user_login && $bpassword)
{
	...
	else
	{
		...
		if ($row['password']==$bpassword)
		{
			...
			echo "<br>Address: ".$rowInfo['address'] ."<hr>";
